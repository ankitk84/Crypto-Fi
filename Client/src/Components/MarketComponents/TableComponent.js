...
const SavedBtn = ({ data }) => {
  ...
  const svgStyle = {
    width: '30px',
    height: '30px',
    fill: allCoins.includes(data.id) ? 'cyan' : 'gray',
  };
...
// };
  ...
  return (
    <>
      ...
      {cryptoData ? (
        <TableContainer component={Paper} className="mt-3 p-5 "  >
                ...
                .map((data) => {
                  return (
                    <TableRow key={data.id}>
                      ...
                      <TableCell align='right'>
                        <PlainChart id={data.id} 
                        profit={data.price_change_percentage_1h_in_currency} 
                        />
                      </TableCell>
                    </TableRow>
                  );
